...
public class StackManager : MonoBehaviour
{
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("Ball"))
        {
            ...
            PlayerManager.playerManagerInstance.balls.Add(other.transform);
        }
        ...
        if (other.transform.CompareTag("Green"))
        {
            ...
            for (int i = 0; i < addBalls; i++)
            {
                GameObject ball = Instantiate(PlayerManager.playerManagerInstance.newBall,
                    PlayerManager.playerManagerInstance.balls.ElementAt(PlayerManager.playerManagerInstance.balls.Count - 1).position +
                     new Vector3(0f, 0f, 0.5f), Quaternion.identity);

                PlayerManager.playerManagerInstance.balls.Add(ball.transform);
            }
